...
from networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP, ResNet18_AP, CarRacing
...
def get_racing_dataset(data_path, args=None):
    class_map = None
    loader_train_dict = None
    class_map_inv = None
    channel = 3
    im_size = (32, 32)
    num_classes = 6
    mean = [0.3552, 0.6498, 0.3555]
    std = [0.1375, 0.2922, 0.1388]
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std),  transforms.Resize(im_size)]) #
    dst_train = CarRacingDataset(os.path.join(data_path, 'CarRacing'), transform=transform)
    class_names = dst_train.classes
    class_map = {x: x for x in range(num_classes)}

    return channel, im_size, num_classes, class_names, dst_train, loader_train_dict, class_map, class_map_inv
...
class CarRacingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        image_path = os.path.join(self.dir, f'observation_{idx}.npy')
        action_path = os.path.join(self.dir, f'action_{idx}.npy')
        image = np.load(image_path)
        action = np.load(action_path)
        if self.transform:
            image = self.transform(image)  # .permute(2, 0, 1)
        action = torch.Tensor(action)
        label = action_to_class(action)
        sample = (image, label)
        return sample
...
def action_to_class(action):
    ...
    if torch.equal(action, action_class_mapping["nothing"]):
        return 0
    elif torch.equal(action, action_class_mapping["gas"]):
        return 1
    elif torch.equal(action, action_class_mapping["steer_right"]):
        return 2
    elif torch.equal(action, action_class_mapping["steer_left"]):
        return 3
    elif torch.equal(action, action_class_mapping["steer_left and brake"]):
        return 4
    elif torch.equal(action, action_class_mapping["steer_right and brake"]):
        return 5
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, texture=False):
    loss_total, acc_avg, num_exp = 0, 0, 0
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        lab = datum[1].to(args.device) # for soft labels should be float
        ...
        if mode == 'train':
            ...
            acc = np.sum(
                np.equal(np.argmax(output.cpu().data.numpy(), axis=-1), lab.cpu().data.numpy())) # for softlabels np.argmax(lab.cpu().data.numpy(), axis=1)
        else:
            loss = criterion(output, torch.nn.functional.one_hot(lab.to(torch.int64), num_classes=10))
            acc = np.sum(np.equal(np.argmax(output.cpu().data.numpy(), axis=-1), lab.cpu().data.numpy()))
        loss_total += loss.item()  #* n_b
        acc_avg += acc
        num_exp += n_b
    ...
    return loss_total, acc_avg


import gym
def evaluate_racing(net):
    net.eval()
    ...
    resize = transforms.Resize((new_dim, new_dim))
    for episode in range(10):
        ...
        for t in range(600):
            env.render()
            action_scores = net(resize(norm(torch.Tensor(
                np.ascontiguousarray(observation[None])).permute(0, 3, 1, 2))).to(device))

            steer, gas, brake = net.scores_to_action(action_scores)
